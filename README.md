...
- The TTGO's built-in button (GPIO 35) cycles the TFT through eight screens: live data, WiFi MAC address, a connection-status screen showing ESP-NOW LINK, j4_stepper_neck, j4_stepper_eyes, j4_talk, j4_display_left, and j4_display_right as CONNECTED (green) / DISCONNECTED (red), then five more, one per ADS1115 in the system: ADS_01 through ADS_04 on this board, plus the fifth on j4_display_right (IRIS, COLOR, BRIGHTNESS, VOLUME) fed by its `P:` lines rather than by I2C. Each shows eight values: every channel's live **raw** count, and under it what that channel drives plus the processed value that leaves this board. Raw is what you need to calibrate a fader window or a joystick centre, so this replaces putting a laptop on the I2C bus
